  ...
  <ion-custom-body class="justify-center mt-6">
    <ion-content :fullscreen="true">
      <form @submit.prevent="checkAndJoin" class="mb-24">
        ...
        <div>
          <ion-item>
            <ion-label position="stacked">지도사 자격증</ion-label>
            <ion-input v-model="joinFormState.license" type="text" placeholder="지도사 자격증을 입력해주세요."></ion-input>
            <input class="mt-3" ref="licenseImgElRef" type="file">
          </ion-item>
        </div>
        <div class="py-1 px-4">
...
export default {
  ...
  setup() {
    ...
    function checkAndJoin() {
      ...
      async function join(loginId: string, loginPw: string, name: string, cellphoneNo: string, email: string, region: string, license: string, genFileIdsStr: string) {
        const axRes = await  mainService.expert_doJoin(loginId, loginPw, name, cellphoneNo, email, region, license, genFileIdsStr);
      ...
      const startJoin = (genFileIdsStr: string) =>{
          join(loginId, loginPw, name, cellphoneNo, email, region, license, genFileIdsStr);
      }
